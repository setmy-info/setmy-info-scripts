...
RUN ls -la
RUN apt install ./setmy-info-scripts-0.94.0.noarch.deb
...
COPY changelog ./
RUN dos2unix **/* && dos2unix ./configure && dos2unix ./src/main/sh/build/packages-build.sh &&  chmod ugoa+x ./src/main/sh/build/packages-build.sh
COPY --from=deb_build_image /var/opt/setmy.info/build/setmy-info-scripts-0.94.0.noarch.deb /var/opt/setmy.info/build/setmy-info-scripts-0.94.0.noarch.deb

RUN ./src/main/sh/build/packages-build.sh
RUN ls -la
#RUN rpm -i ./setmy-info-scripts-0.94.0.noarch.rpm
#RUN ls -la /opt/setmy.info
